...
void do_channel_get_members (struct command *command, int arg_num, struct arg args[], struct in_ev *ev) {
  assert (arg_num == 3);
  if (ev) { ev->refcnt ++; }
  tgl_do_channel_get_members (TLS, args[0].peer_id, args[1].num == NOT_FOUND ? 0 : args[1].num, args[2].num == NOT_FOUND ? 0 : args[2].num, 0, print_user_list_gw, ev);
}

void do_channel_get_admins (struct command *command, int arg_num, struct arg args[], struct in_ev *ev) {
  assert (arg_num == 3);
  if (ev) { ev->refcnt ++; }
  tgl_do_channel_get_members (TLS, args[0].peer_id, args[1].num == NOT_FOUND ? 0 : args[1].num, args[2].num == NOT_FOUND ? 0 : args[2].num, 1, print_user_list_gw, ev);
}

void do_channel_get_kicked (struct command *command, int arg_num, struct arg args[], struct in_ev *ev) {
  assert (arg_num == 3);
  if (ev) { ev->refcnt ++; }
  tgl_do_channel_get_members (TLS, args[0].peer_id, args[1].num == NOT_FOUND ? 0 : args[1].num, args[2].num == NOT_FOUND ? 0 : args[2].num, 3, print_user_list_gw, ev);
}

void do_channel_get_bots (struct command *command, int arg_num, struct arg args[], struct in_ev *ev) {
  assert (arg_num == 3);
  if (ev) { ev->refcnt ++; }
  tgl_do_channel_get_members (TLS, args[0].peer_id, args[1].num == NOT_FOUND ? 20 : args[1].num, args[2].num == NOT_FOUND ? 0 : args[2].num, 4, print_user_list_gw, ev);
}
...
struct command commands[MAX_COMMANDS_SIZE] = {
  ...
  {"channel_get_admins", {ca_channel, ca_number | ca_optional, ca_number | ca_optional, ca_none}, do_channel_get_admins, "channel_get_admins <channel> [limit=0] [offset=0]\tGets channel admins", NULL},
  {"channel_get_members", {ca_channel, ca_number | ca_optional, ca_number | ca_optional, ca_none}, do_channel_get_members, "channel_get_members <channel> [limit=0] [offset=0]\tGets channel members", NULL},
  {"channel_get_kicked", {ca_channel, ca_number | ca_optional, ca_number | ca_optional, ca_none}, do_channel_get_kicked, "channel_get_kicked <channel> [limit=0] [offset=0]\tGets channel kicked", NULL},
  {"channel_get_bots", {ca_channel, ca_number | ca_optional, ca_number | ca_optional, ca_none}, do_channel_get_bots, "channel_get_bots <channel> [limit=20] [offset=0]\tGets channel bots", NULL},
